...
The checkpoints are **not stored in this GitHub repository**. They are downloaded automatically the first time a model is used (≈210 MB per model, takes around 30 seconds on a typical connection) and cached locally by Hugging Face under `~/.cache/huggingface/hub/`. Subsequent invocations reuse the cached file and run in seconds.
...
Run inference on all sample images with the best (default) model. By default, it uses GPU if available and falls back to CPU otherwise:

```bash
cd pix2pix-monuseg-demo
python demo/demo.py --input demo/samples --output demo/outputs
```

Run inference on a single image:

```bash
python demo/demo.py --input demo/samples/image_099.png --output demo/outputs/image_099_demo.png
```
...
```bash
python demo/demo.py --input demo/samples --device cpu
```
...
The images in `demo/samples/` are examples from the MoNuSeg test split. None of them was seen by the models during training or validation.
